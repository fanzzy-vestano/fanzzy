...
export default function Home() {
  ...
  return (
    <main className="site-shell">
      <div className="announcement"><span>THE MIDNIGHT EDIT</span><strong>Complimentary shipping on orders above ₹999</strong><button onClick={() => announce("You're all set for the Midnight Edit")}>Explore now&nbsp; ↗</button></div>

      <header className="site-header">
        <a href="#top" className="wordmark" aria-label="Lustre home"><span>lustre</span><small>objects of allure</small></a>
      ...
      <section className="hero" id="top">
        ...
        <div className="hero-art"><div className="hero-orbit orbit-one" /><div className="hero-orbit orbit-two" /><div className="hero-arch"><img src="https://images.unsplash.com/photo-1599643478518-a784e5dc4c8f?auto=format&fit=crop&w=1200&q=90" alt="Gold necklace arranged on a warm stone surface" /></div><span className="hero-note">01 / 03<br /><i>new collection</i></span></div>
        <div className="hero-bottom"><span>Scroll to discover</span><span className="scroll-line" /><span>Est. 2019 · New Delhi</span></div>
      </section>

      <section className="manifesto"><p className="eyebrow">THE LUSTRE STANDARD</p><h2>Jewellery with a point of view.<br /><em>Made for your everyday extraordinary.</em></h2><p className="manifesto-copy">Lustre is a study in contrast — soft and sculptural, familiar and unexpected. Every piece is made in small batches with considered materials and a little bit of magic.</p></section>

      <section className="section-block" id="categories"><div className="section-heading"><div><p className="eyebrow">SHOP BY MOOD</p><h2>Find your <em>signature.</em></h2></div><a className="text-link" href="#shop">View all categories <span>↗</span></a></div><div className="category-grid">{categories.map((category, index) => <button className={`category-card category-${index + 1}`} key={category.name} onClick={() => { setActiveCategory(category.name); document.getElementById("shop")?.scrollIntoView({ behavior: "smooth" }); }}><img src={category.image} alt="" /><span className="category-overlay" /><span className="category-info"><strong>{category.name}</strong><small>{category.count} &nbsp;↗</small></span></button>)}</div></section>
      ...
      <section className="editorial" id="story"><div className="editorial-image"><img src="https://images.unsplash.com/photo-1515562141207-7a88fb7ce338?auto=format&fit=crop&w=1100&q=85" alt="Close-up of sculptural gold jewelry" /><span>THE ART OF<br /><em>ADORNMENT</em></span></div><div className="editorial-copy"><p className="eyebrow">A NOTE FROM THE STUDIO</p><h2>Less noise.<br /><em>More meaning.</em></h2><p>There is beauty in the in-between. The way a quiet chain layers with your favourite shirt. A ring that becomes part of your hand. Lustre is made for these small rituals — the ones that make a day feel like yours.</p><a className="button button-dark" href="#footer">Read our story <span>↗</span></a><div className="editorial-sign">L / 19<br /><small>Objects of allure</small></div></div></section>

      <section className="offer-banner"><div><p className="eyebrow light">A LITTLE EXTRA</p><h2>Your first piece<br /><em>is on us.</em></h2></div><div><p>Take 10% off your first order with code <strong>HELLOLUSTRE</strong>.</p><button className="button button-light" onClick={() => { navigator.clipboard?.writeText("HELLOLUSTRE"); announce("Code copied: HELLOLUSTRE"); }}>Copy code <span>↗</span></button></div></section>

      <section className="newsletter"><div><p className="eyebrow">THE LUSTRE LETTER</p><h2>A little light<br /><em>in your inbox.</em></h2></div><form onSubmit={(event) => { event.preventDefault(); if (email) setSubscribed(true); }}><p>New drops, studio notes, and 10% off your first order — no noise, promise.</p><div className="email-line"><input type="email" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="Your email address" aria-label="Your email address" required /><button aria-label="Subscribe to newsletter">↗</button></div>{subscribed && <span className="success-message">You’re on the list. Welcome to Lustre.</span>}</form></section>

      <footer className="site-footer" id="footer"><div className="footer-brand"><a href="#top" className="wordmark wordmark-light"><span>lustre</span><small>objects of allure</small></a><p>Quietly remarkable jewellery<br />for all your becoming.</p></div><div><p className="eyebrow light">Explore</p><a href="#shop">New arrivals</a><a href="#shop">Bestsellers</a><a href="#categories">Collections</a><a href="#shop">Gift cards</a></div><div><p className="eyebrow light">Need a hand?</p><a href="#footer">Contact us</a><a href="#footer">Shipping & returns</a><a href="#footer">Care guide</a><a href="#footer">FAQs</a></div><div><p className="eyebrow light">Follow along</p><a href="#footer">Instagram ↗</a><a href="#footer">Pinterest ↗</a><a href="#footer">WhatsApp ↗</a><p className="footer-small">Made with intention in India.<br />© Lustre 2024</p></div><div className="footer-bottom"><span>Privacy</span><span>Terms</span><span>Accessibility</span><span>India / INR ₹</span></div></footer>

      <button className="whatsapp-float" onClick={() => announce("We'll be in touch shortly")}>✦ <span>Chat with us</span></button>

      {searchOpen && <div className="overlay search-overlay" role="dialog" aria-modal="true" aria-label="Search"><div className="overlay-top"><span className="wordmark"><span>lustre</span></span><button onClick={() => setSearchOpen(false)}>Close&nbsp; ×</button></div><div className="search-content"><p className="eyebrow">SEARCH THE COLLECTION</p><div className="large-search"><input autoFocus value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Try “gold hoops”" /><span>⌕</span></div>{search && <div className="search-results">{filteredProducts.length ? filteredProducts.map((product) => <button key={product.id} onClick={() => { setQuickProduct(product); setSearchOpen(false); }}><img src={product.image} alt="" /><span><strong>{product.name}</strong><small>{product.category} · {formatINR(product.price)}</small></span><b>↗</b></button>) : <p className="muted">No pieces found. Try another search.</p>}</div>}{!search && <div className="search-suggestions"><span>Trending now</span><button onClick={() => setSearch("hoops")}>Hoops</button><button onClick={() => setSearch("pearl")}>Pearls</button><button onClick={() => setSearch("chain")}>Chains</button></div>}</div></div>}
