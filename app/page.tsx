...
export default function Home() {
  ...
  const sendOtp = async () => {
    ...
    try {
        ...
        setAuthMessage(
          result.error
            || (response.status === 404 || response.status === 405
              ? "Voice OTP login is unavailable on this deployment. Please try again later."
              : response.status >= 500
                ? "The voice OTP service is temporarily unavailable. Please try again in a moment."
                : "We could not start the voice OTP call."),
        );
        return;
      }
      setOtpSent(true);
      ...
      setOtpCooldown(60);
      setAuthMessage("Voice OTP call started. Answer the call to hear your code.");
    } catch {
  ...
  const verifyOtp = async () => {
    ...
    if (!token) {
      setAuthMessage("Enter the 6-digit verification code from the call.");
      return;
    }
    setAuthLoading(true);
    setAuthMessage("");
  ...
  const openOrders = () => {
    ...
    setAuthMessage("Sign in with a one-time voice-call code to view your orders.");
    setAuthOpen(true);
  };
  const applyCoupon = () => {
  ...
  const persistPaidOrder = async (newOrder: CustomerOrder) => {
    if (!authUser) {
      ...
      setAuthMessage("Sign in with a one-time voice-call code before placing an order.");
      setAuthOpen(true);
      return;
    }
  ...
  return (
    <main className="site-shell" id="top">
      ...
      {authOpen && <div className="drawer-backdrop auth-backdrop" onClick={closeAuth}><section className="auth-modal" role="dialog" aria-modal="true" aria-labelledby="auth-title" onClick={(event) => event.stopPropagation()}><button className="modal-close" aria-label="Close sign in" onClick={closeAuth}>×</button><p className="eyebrow">SECURE MOBILE LOGIN</p><h2 id="auth-title">Continue with your mobile number.</h2><p className="auth-intro">Enter your mobile number and receive a one-time code by voice call. No password is required.</p><div className="otp-auth-form"><label>Mobile number<input type="tel" value={authPhone} onChange={(event) => { setAuthPhone(event.target.value); setAuthOtp(""); setOtpSent(false); setOtpCooldown(0); }} placeholder="+91 98765 43210" inputMode="tel" autoComplete="tel" disabled={authLoading} /></label>{otpSent && <label>6-digit verification code<input value={authOtp} onChange={(event) => setAuthOtp(event.target.value.replace(/\D/g, "").slice(0, 6))} inputMode="numeric" autoComplete="one-time-code" maxLength={6} placeholder="Enter code from the call" disabled={authLoading} onKeyDown={(event) => { if (event.key === "Enter") void verifyOtp(); }} /></label>}{otpSent ? <><button className="google-sign-in" type="button" onClick={() => void verifyOtp()} disabled={authLoading}><span className="email-otp-mark" aria-hidden="true">✦</span>{authLoading ? "Please wait…" : "Verify voice code"}<span aria-hidden="true">↗</span></button><div className="otp-fallback-actions"><button className="auth-resend" type="button" onClick={() => void sendOtp()} disabled={authLoading || otpCooldown > 0}>{otpCooldown > 0 ? `Call again in ${otpCooldown}s` : "Call again with OTP"}</button></div></> : <div className="otp-channel-actions"><button className="google-sign-in auth-voice-button" type="button" onClick={() => void sendOtp()} disabled={authLoading}><span className="email-otp-mark" aria-hidden="true">☎</span>{authLoading ? "Please wait…" : "Call me with OTP"}<span aria-hidden="true">↗</span></button></div>}</div>{authMessage && <p className="auth-message" role="alert">{authMessage}</p>}<p className="auth-note">Your orders and cart are saved to this verified mobile number.</p></section></div>}
