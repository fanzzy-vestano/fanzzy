...
export default function AdminPage() {
  ...
  const notify = (message: string) => {
    if (message === "Preview opened") { window.location.assign("/"); return; }
    if (message === "New product form opened" || message.endsWith(" selected")) setActive("Products");
    setToast(message);
    window.setTimeout(() => setToast(""), 2200);
  };
  return <main className="admin-shell">
    <aside className="admin-sidebar"><div className="admin-brand"><a href="/" className="wordmark"><img src="/fanzzy-mark.png" alt="Fanzzy" className="brand-logo" /><small>control room</small></a><span className="live-dot">LIVE</span></div><div className="admin-profile"><div className="avatar">VE</div><div><strong>Vestano</strong><small>Super admin</small></div><button onClick={() => notify("Profile menu opened")}>⌄</button></div><p className="admin-label">Workspace</p><nav className="admin-nav">{menu.map((item) => <button key={item.label} className={active === item.label ? "active" : ""} onClick={() => setActive(item.label)}><span className="nav-icon">{item.icon}</span>{item.label}{item.count && <b>{item.count}</b>}</button>)}</nav><div className="sidebar-bottom"><button onClick={() => setActive("Settings")}><span className="nav-icon">⚙</span>Settings</button><a href="/"><span className="nav-icon">↩</span>View storefront</a></div></aside>
