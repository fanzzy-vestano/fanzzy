...
const paymentAddress = (payment) => typeof payment?.notes?.address === "string" && payment.notes.address.trim() ? payment.notes.address.trim() : undefined;
...
while (true) {
  ...
  for (const payment of payments) {
    ...
    if (paymentAddress(payment)) order.address = paymentAddress(payment);
    if (customerEmail(payment.email)) order.email = customerEmail(payment.email);
    if (!customerEmail(payment.email) && order.email?.toLowerCase().endsWith("@razorpay.com")) delete order.email;
    if (!customerEmail(payment.email) && order.customerName === payment.email?.split("@")[0]) order.customerName = "Razorpay customer";
